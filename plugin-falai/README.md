...
This plugin enables AI-powered image and video generation using fal.ai's API. It provides actions for generating images from text prompts and creating videos from images.

## Features

- **Text-to-Image Generation**: Generate high-quality images from text descriptions using models like FLUX, Stable Diffusion, and more
- **Image-to-Video Generation**: Animate static images by converting them into videos using AI models

## Installation

```bash
npm install plugin-falai
# or
bun add plugin-falai
```

## Configuration

You need to set up your fal.ai API key to use this plugin:
...
Users can generate images by asking the agent:

- "generate image of a cute puppy playing in a garden"
- "create a picture of a futuristic city at night"
- "show me an image of a sunset over mountains"
- "make an image of a robot in space"

### Generating Videos from Images

Users can generate videos from images by:

1. Attaching an image and asking:
   - "generate video from this image"
   - "animate this image with a gentle breeze effect"
   - "create a video from this picture"

2. Or providing an image URL:
   - "create video from https://example.com/image.jpg with cinematic motion"

## Supported Models

### Image Generation Models
- `fal-ai/flux/dev` (default) - FLUX.1 [dev] - High-quality text-to-image generation
- `fal-ai/recraft-v3` - Recraft V3 with advanced capabilities
- `fal-ai/stable-diffusion-v35-large` - Stable Diffusion 3.5 Large
- And many more available on [fal.ai models page](https://fal.ai/models)

### Video Generation Models
- `fal-ai/minimax-video/image-to-video` (default) - MiniMax Video model
- `fal-ai/luma-dream-machine` - Luma Dream Machine v1.5
- `fal-ai/kling-video/v1/standard` - Kling 1.0
- And more available on [fal.ai models page](https://fal.ai/models)
...
src/
├── services/
│   └── FalService.ts      # Service for fal.ai API integration
├── actions/
│   ├── generateImage.ts  # Action for text-to-image generation
│   └── generateVideo.ts # Action for image-to-video generation
...
describe('Plugin Configuration', () => {
  it('should have correct plugin metadata', () => {
    expect(starterPlugin.name).toBe('plugin-falai');
  });
});
```
